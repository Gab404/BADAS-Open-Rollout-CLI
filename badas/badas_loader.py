...
class BADASModel:
    ...
    def predict(self, video_path: str, real_time: bool = False, show_time: bool = False, attn_rollout: bool = False, grad_rollout: bool = False) -> List[float]:
            """
            Predict collision probability for each frame window in video.
            """
            with torch.no_grad():
                # Passer le paramètre au modèle VJEPA
                predictions = self.model.predict(video_path, real_time=real_time, show_time=show_time, attn_rollout=attn_rollout, grad_rollout=grad_rollout)
...
def preprocess_video(video_path: str, 
    ...
    if not os.path.exists(video_path):
        raise FileNotFoundError(f"Impossible de trouver le fichier vidéo : {os.path.abspath(video_path)}")

    cap = cv2.VideoCapture(video_path)
    
    if not cap.isOpened():
        raise ValueError(f"Erreur: OpenCV n'arrive pas à ouvrir {video_path}. Vérifiez le format/codec.")
    ...
    # 2. VÉRIFICATION AVANT PADDING (C'est ici que ça plantait)
    if len(frames) == 0:
        raise ValueError("Aucune frame n'a pu être extraite de la vidéo. Le fichier est peut-être corrompu ou vide.")
